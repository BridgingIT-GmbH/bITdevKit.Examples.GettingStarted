...
/// <summary>
/// Strongly typed configuration model for the TestCore.
...
/// <code>
/// // In your module bootstrap (e.g. TestCore.cs)
/// var moduleConfiguration = this.Configure&lt;TestCoreConfiguration, TestCoreConfiguration.Validator&gt;(
///     services, configuration);
/// </code>
/// - Binds the <c>"TestCore"</c> section of <c>appsettings.json</c> into an instance of <see cref="TestCoreConfiguration"/>.
/// - Validates it with <see cref="Validator"/> (FluentValidation).
/// - Registers it into the DI container for <c>IOptions&lt;TestCoreConfiguration&gt;</c>.
...
{
    /// <summary>
    /// Gets or sets the connection strings available to the TestCore.
    ...
    /// <summary>
    /// FluentValidation validator for <see cref="TestCoreConfiguration"/>.
    /// Ensures that the configuration is not null or empty and includes a
    /// connection string entry named <c>"Default"</c>.
    /// </summary>
    public class Validator : AbstractValidator<TestCoreConfiguration>
    {
        public Validator()
        {
