...
$projects | ForEach-Object -Begin {
  ...
  $razorFiles = @(
    Get-ChildItem -Path $projectDir -Recurse -Filter "*.razor" |
      Where-Object { Should-ProcessFile -file $_ -skipGenerated $SkipGeneratedFiles -excludePatterns $ExcludePatterns }
  )
  $totalFiles = $sourceFiles.Count + $razorFiles.Count
  $currentFile = 0
  ...
  $razorFiles | ForEach-Object {
    ...
    # Optionally, apply code cleanup to .razor files if desired (can be customized)
    $content = Remove-CodeElements -content $content `
      -stripRegions $StripRegions `
      -stripComments $StripComments `
      -stripEmptyLines $StripEmptyLines `
  ...
  foreach ($file in $processedFiles) {
    ...
    if ($file.Type -eq 'razor') {
      $markdown += '```csharp' # Razor files are typically C# with markup
    } else {
      $markdown += '```csharp'
    }
    $markdown += "`n"
...
Size Reduction:      $(
    if ($stats.OriginalSize -eq 0) {
        if ($stats.FinalSize -eq 0) { "0.0" } else { "N/A" }
    } else {
        [math]::Round(100 - ($stats.FinalSize / $stats.OriginalSize * 100), 1)
    }
)%
